...
@implementation ReciveRequestView
...
-(void)callRequestParsing
{
  ...
  //  NSLog(@"user id %@",[[myGlobalArray objectAtIndex:0]valueForKey:@"intID"]);
    ...
    // h/ttp://humsomething.netsmartz.us/HumServices.svc/getFriendRequestsreceived/intUserID=69
    
   // ht tp://humsomething.netsmartz.us/HumServices.svc/getFriendRequestsreceived/intUserID=100
    // Create the Request.
    NSURLRequest *request = [NSURLRequest requestWithURL:
                             [NSURL URLWithString:[NSString stringWithFormat:@"http://humsomething.netsmartz.us/HumServices.svc/getFriendRequestsreceived/intUserID=%@",[[myGlobalArray objectAtIndex:0]valueForKey:@"intID"]]]
...

{
    ...
    if ([elementName isEqualToString:@"acceptrejectfriendResult"])
    {
        ...
        if([capturedCharacters isEqualToString:@"Success"])
        {
            if (i==0) 
            {
                UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Message!" message:[NSString stringWithFormat:@"%@ is added to friend circle",strName] delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
                [alert show];
                [alert release];
            }
            else {
                UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Message!" message:@"Friend request is removed" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
                [alert show];
                [alert release];
...
{ 
    ...
    requestButton.frame = CGRectMake(240,15 ,23,23);
    ...
    btnAccept.frame = CGRectMake(200,12,30,30);
    [btnAccept setImage:[UIImage imageNamed:@"addrequestBtn.png"] forState:UIControlStateNormal];
    
    btnAccept.tag=indexPath.row;
    [btnAccept addTarget:self action:@selector(acceptFriendRequest:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)removeFriendRequest:(UIButton *)tag
{
   ...
   // ht tp://humsomething.netsmartz.us/HumServices.svc/acceptrejectfriend/intUserID=100,intFriendID=84,intStatus=0
    i=1;
   //  NSLog(@"%@ %@",[[myGlobalArray objectAtIndex:0]valueForKey:@"intID"],[[resultArrayFinal objectAtIndex:tag.tag]valueForKey:@"intID"]);
    
NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://humsomething.netsmartz.us/HumServices.svc/acceptrejectfriend/intUserID=%@,intFriendID=%@,intStatus=%@",[[myGlobalArray objectAtIndex:0]valueForKey:@"intID"],[[resultArrayFinal objectAtIndex:tag.tag]valueForKey:@"intID"],@"0"]]
...
-(void)acceptFriendRequest:(UIButton *)tag
{
    i=0;
  //  NSLog(@"userid %@",[[myGlobalArray objectAtIndex:0] valueForKey:@"intID"]);
    
    //h ttp://humsomething.netsmartz.us/HumServices.svc/acceptrejectfriend/intUserID=66,intFriendID=69,intStatus=0
    // Create the Request.
    
     //://humsomething.netsmartz.us/HumServices.svc/acceptrejectfriend/intUserID=100,intFriendID=84,intStatus=1
    
    
  // NSLog(@"%@ %@",[[myGlobalArray objectAtIndex:0] valueForKey:@"intID"],[[resultArrayFinal objectAtIndex:tag.tag] valueForKey:@"intID"]);
    NSURLRequest *request = [NSURLRequest requestWithURL:
                             [NSURL URLWithString:[NSString stringWithFormat:@"http://humsomething.netsmartz.us/HumServices.svc/acceptrejectfriend/intUserID=%@,intFriendID=%@,intStatus=%@",[[myGlobalArray objectAtIndex:0] valueForKey:@"intID"],[[resultArrayFinal objectAtIndex:tag.tag] valueForKey:@"intID"],@"1"]]
                                             cachePolicy:NSURLRequestUseProtocolCachePolicy
                                         timeoutInterval: 30.0];
